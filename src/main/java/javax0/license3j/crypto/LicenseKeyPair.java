...
public class LicenseKeyPair {
    ...
    /**
     * Get the byte representation of the private key as it is returned by the underlying security library. This is NOT
     * the byte array that contains the algorithm at the start. This is the key in raw format.
     *
    ...
    /**
     * Get the byte representation of the public key as it is returned by the underlying security library. This is NOT
     * the byte array that contains the algorithm at the start. This is the key in raw format.
     *
     * @return the key as bytes
     */
    ...
    public static class Create {
         ...
         */
        public static LicenseKeyPair from(final PublicKey publicKey,
         ...
         * contain the mode and the padding as {@code algorithm/mode/padding}. Note that the mode and the padding is not
         * needed for the key generation. Nevertheless these are also stored in the generated {@link LicenseKeyPair} and
         * will be used to sign/verify the license.
         * <p>
         ...
         *                                  encryption provider
         */
        public static LicenseKeyPair from(final String cipher,
                                          final int size)
        ...
        /**
         * Create a new {@link LicenseKeyPair} that contains only one of the keys, either the private or the public. The
         * key is provided in binary format as a byte array.
         * 
         * @param encoded the key encoded as byte array. This is the format that contains the algorithm at the start of
         *                the byte array as zero byte terminated string.
         ...
         * 
         * @param privateEncoded the private key encoded including the cipher specification with a zero byte terminated
         *                       at the start of the byte array
         * @param publicEncoded  the public key encoded including the cipher specification with a zero byte terminated
         *                       at the start of the byte array
         * @return the newly created {@link LicenseKeyPair}
         * @throws NoSuchAlgorithmException if the algorithm in the encoded key is not implemented by the actual
         *                                  encryption provider
        ...
        private static PublicKey getPublicEncoded(byte[] buffer)
                throws NoSuchAlgorithmException, InvalidKeySpecException {
        ...
        private static PrivateKey getPrivateEncoded(byte[] buffer)
                throws NoSuchAlgorithmException, InvalidKeySpecException {
